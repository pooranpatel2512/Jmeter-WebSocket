package com.pooran.jmeter.protocol.websocket.sampler;
 ...
 * @author kawasima (small modification by Pooran Patel)
 */
...
public class WebSocketSampler extends AbstractSampler implements TestStateListener {
    ...
    private static final Set<String> APPLIABLE_CONFIG_CLASSES = new HashSet<String>(
            Arrays.asList(new String[]{
                    "com.pooran.jmeter.protocol.websocket.control.gui.WebSocketSamplerGui",
                    "org.apache.jmeter.config.gui.SimpleConfigGui"}));
    
    private static boolean TEST_STARTED = false;

    private static boolean TEST_ENDED = false;
    ...
    private static Future<WebSocket.Connection> futureConnection = null;
    private static ConcurrentHashMap<String, Future<WebSocket.Connection>> samplerConnections =
    		new ConcurrentHashMap<String, Future<WebSocket.Connection>>();
    ...
    public void initialize() throws Exception {
        ...
        webSocketClient.setMaxTextMessageSize(20000 * 1024);
        ...
        if(!samplerConnections.containsKey(threadName)) {
        	log.debug("New websocket connection for thread = " + threadName);
            futureConnection = webSocketClient.open(uri, new WebSocket.OnTextMessage() {

            	public void onMessage(String receivedMessage) {
                    if (regex == null || regex.matcher(receivedMessage).find()) {
                        log.debug("onMessageReceive: message received for thread = " + threadName + " of length = " + receivedMessage.length() + 
                        		" for sampler = " + getName());
                        log.debug("onMessageReceive: message = " + receivedMessage);
                    }
                }
                ...
                public void onClose(int i, String s) {
                    log.debug("Disconnect " + threadName + " int i = " + i + ", String = " + s);
                }
            });
            synchronized(samplerConnections) {
            	samplerConnections.put(threadName, futureConnection);
			}
        } else {
        	log.debug("Old websocket connection for thread = " + threadName);
        }
        connection = samplerConnections.get(threadName).get();
        initialized = true;
    }
    ...
    public SampleResult sample(Entry entry) {
        ...
        try {
            ...
            res.setResponseData("Message is sent, But at the moment it does not show received message. see log of the "
            		+ "plgin for received messages", getContentEncoding());
            isOK = true;
        } catch (Exception e) {
...
//        		webSocketClientFactory.setBufferSize(30000);
        		log.debug("Web socket client factory starting..");
        		TEST_STARTED = true;
        	}
        } catch(Exception e) {
    ...
    public void testEnded(String host) {
        try {
        	if(!TEST_ENDED) {
        		log.debug("Web socket client factory starting..");
        		log.debug("Closing all web socket connections..");
        		Set<String> thredNames = samplerConnections.keySet();
        		Iterator<String> iterator = thredNames.iterator();
        		for (String threadName : thredNames) {
        			samplerConnections.get(threadName).get().close();
        		}
        		webSocketClientFactory.stop();
        		log.debug("Web socket client factory stopped..");
        		TEST_ENDED = true;
        	}
        } catch (Exception e) {
